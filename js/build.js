        ...
        export const ServerData = new(function(){
            ...
            this.pausePage = function(c){
                ...
                if(run){
                    let p = `<img id = 'pause-dimension' src = 'http://localhost/jobs/image/load.gif' class = 'load-pause' >
                ...
                if(data.length > 0){
                    let users = data.map( h =>
                        `<section style = 'width : 96%; height:250px; margin:2%;'>
                            <img src = '${ h.image || "http://localhost/jobs/image/avatar.jpg" }' style = 'float:left;width:250px;height:100%;'>
                    ...
                    return users.join(',')
                     ...
                     return `<img src = 'http://localhost/jobs/image/undraw_lost_re_xqjt.svg' class = 'avatar-undraw'>
        ...
        const buildSelect = (e) => {
            ...
            $(id).select2({
                ...
                if(fill == 1){
                    ...
                    let users_id = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : false, 'body' : { 'users_search' : true, 'data' : filter, 'table' : 'users' }, 'data' : 'json'})
                    $('#view_users').html(ServerData.users_string(users_id.users))
                }
                if(fill == 2){
                    let filter = [{ 'cell' : ['employerId'], 'data' : id }]
                    let employer_id = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : false, 'body' : { 'users_search' : true, 'data' : filter, 'table' : 'employers' }, 'data' : 'json'})
        ...
        const plotView = async(e) => {
            ...
            let all_data = {};
            if(channel == 1)
                all_data = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : true, 'body' : { 'jobs_name' : true }, 'data' : 'json' })

            if(channel == 2)
                all_data = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : true, 'body' : { 'users_name' : true }, 'data' : 'json' })

            if(channel == 3)
                all_data = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : true, 'body' : { 'record_name' : true }, 'data' : 'json' })
        ...
        export const retrieveContent = async(user,content,retrieve) => {
            ...
            if(retrieve == 0){
                let obtain_plot = await ServerData.bindAuth({ 'method' : 'POST', 'link' : `http://localhost/jobs/php/index.php`, 'header' : false, 'body' : { 'profile' : true }, 'data' : 'json'})
            ...
            if(retrieve == 2){
                const all_data = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : true, 'body' : { 'users_name' : true }, 'data' : 'json' })
                $('#view_users').html(ServerData.users_string(all_data.all))
            }
            if(retrieve == 3){
                const all_data = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : true, 'body' : { 'applications' : true }, 'data' : 'json' })
                if(all_data.jobs.length > 0){
                    let all_string = all_data.jobs.map( a =>
                        `<div id = 'hold-job'>
                            ...
                            <h3>${ (all_data.approved)?"approved":"pending approval" }</h3>
            ...
            if(retrieve == 4){
                const all_data = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : true, 'body' : { 'employers_name' : true }, 'data' : 'json' })
                $('#view_users').html(ServerData.users_string(all_data.all))
            }
            if(retrieve == 5){
                const all_data = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : true, 'body' : { 'all_jobs' : true }, 'data' : 'json' })
                if(all_data.jobs.length > 0){
                    let all_string = all_data.jobs.map( a =>
                        `<div id = 'hold-job'>
                            <h2>${ a.job_title }</h2>
            ...
            if(retrieve == 7){
                let all_jobs = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : true, 'body' : { 'jobs_name' : true }, 'data' : 'json' })

                buildSelect({'id' : '#jobs-record', 'placeholder' : 'jobs...', 'data' : all_jobs })

                let all_doctor = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : true, 'body' : { 'doctor_name' : true }, 'data' : 'json' })
            ...
            if(retrieve == 8){
                [
                    ...
                    let graph_data = await ServerData.bindAuth({ 'method' : 'POST', 'link' : `http://localhost/jobs/php/index.php`, 'header' : true, 'body' : { 'graph' : true, 'table' : go_table }, 'data' : 'json' })
                    createGraph({'graph_data' : graph_data, 'id' : go_id, 'text' : go_text, 'label_one' : go_text, 'label_two' : go_text, 'label_three' : go_text })
                })
            }
            if(retrieve == 9){
                let all_jobs = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : true, 'body' : { 'users_name' : true }, 'data' : 'json' })

                buildSelect({'id' : '#select_user', 'placeholder' : 'users...', 'data' : all_jobs, 'fill' : 1})

            }
            if(retrieve == 10){
                let all_employer = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : true, 'body' : { 'employers_name' : true }, 'data' : 'json' })

                buildSelect({'id' : '#select_user', 'placeholder' : 'Employers...', 'data' : all_employer, 'fill' : 2 })

            }
            if(retrieve == 11){
                const all_data = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : true, 'body' : { 'employer_jobs' : true }, 'data' : 'json' })
                if(all_data.jobs.length > 0){
            ...
            if(retrieve == 12){
                 let all_group = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : true, 'body' : { 'job_groups' : true }, 'data' : 'json' })
        ...
        export const editProfile = async(e,n,f,s) => {
           ...
           if(n)
               ...
               let edit = {}
               if(s == 1)
                    edit = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'load' : true, 'header' : false, 'body' : { 'profile-edit' : true, 'data' : e.currentTarget.value, 'cell' : document.querySelectorAll('#profile-tower p')[loop].innerHTML.trim() }, 'data' : 'json'})
               else
                    edit = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'load' : true, 'header' : false, 'body' : { 'profile-edit-employer' : true, 'data' : e.currentTarget.value, 'cell' : document.querySelectorAll('#profile-tower p')[loop].innerHTML.trim() }, 'data' : 'json'})
        ...
        export const apply_package = async(e) => {
            let apply = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : false, 'body' : { 'apply' : true, 'data' : e.currentTarget.attributes[1].value }, 'data' : 'json'})
            if(apply.package)
                jobsContent([5])
        }
        ...
        export const search_employers = async(e) => {
            ...
            let employer_id = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : false, 'body' : { 'users_search' : true, 'data' : filter, 'table' : 'employer' }, 'data' : 'json' })
            $('#view_users').html(ServerData.users_string(employer_id.users))
        }
        ...
        export const search_jobs = async(e) => {
            ...
            let jobs_id = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : false, 'body' : { 'jobs_search' : true, 'data' : jobs }, 'data' : 'json'})
            if(jobs_id){
                if(jobs_id.jobs){
        ...
        export const attach = async() => {
            ...
            let update_pro = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : false, 'body' : frmD, 'data' : 'json', 'way' : true })
        ...
        export const education = async() => {
            ...
            let update_pro = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : false, 'body' : { 'update_user' : true, 'data' : education_data, 'table' : 'education' }, 'data' : 'json'})
            if(update_pro.success){
                $('#ed_inst').val('')
                $('#ed_grade').val('')
                $('#ed_start').val('')
        ...
        export const referees = async() => {
            ...
            let update_pro = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : false, 'body' : { 'update_user' : true, 'data' : referees_data, 'table' : 'referees' }, 'data' : 'json'})
            if(update_pro.success){
                $('#ref_inst').val()
                $('#ref_title').val()
                $('#ref_name').val()
        ...
        export const work = async() => {
            ...
            let update_pro = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : false, 'body' : { 'update_user' : true, 'data' : work_data, 'table' : 'work' }, 'data' : 'json'})
            if(update_pro.success){
                $('#work_inst').val()
                $('#work_start').val()
                $('#work_end').val()
        ...
        export const search_doctor = async(e) => {
            ...
            let doctor_id = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : false, 'body' : { 'doctor_search' : true, 'data' : doctor }, 'data' : 'json'})
            if(doctor_id){
                if(doctor_id.doctor){
        ...
        export const search_records = async(e) => {
            ...
            let record_id = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : false, 'body' : { 'records_search' : true, 'data' : record }, 'data' : 'json'})
            if(record_id){
                if(record_id.record){
        ...
        export const jobs_add = async(e) => {
            ...
            if(!telephone || !email || !name || !address || !img){
                ...
                let jobs = await ServerData.bindAuth({ 'method' : 'POST', 'link' : `http://localhost/jobs/php/index.php`, 'header' : false, 'body' : frmD, 'data' : 'json', 'way' : true })
                if(jobs)
        ...
        export const apply_jobs = async(e) => {
            let apply = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : true, 'body' : { 'apply_jobs' : true, 'id' : e.currentTarget.attributes[1].value }, 'data' : 'json' })
            if(apply.success){
        ...
        export const create_ad = async() => {
            let allow = true;
            let points = []
            document.querySelectorAll('#input-div-internal textarea').forEach( (t,k) => {
                if(!t.value){
                    document.querySelectorAll('#feedback')[k].innerHTML = document.querySelectorAll('#input-div-internal h4')[k].innerHTML + " is not correct";
                    allow = false;
                }else
                    points.push(t.value)
            })
            ...
            if(allow){
                let qualify_arr = []
                document.querySelectorAll('#part-qualify h4').forEach( d => {
                    qualify_arr.push(d.innerHTML)
                })
                points.push($('#ad_field').val().split(',')[1])
                points.push($('#ad_expiration').val())
                points.push(qualify_arr)
                let create = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : true, 'body' : { 'ad' : true, 'value' : points }, 'data' : 'json' })
        ...
        export const candidate_approve = async(e) => {
            let approve = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : true, 'body' : { 'approve' : true, 'id' : e.currentTarget.attributes[1].value }, 'data' : 'json' })
            if(approve){
        ...
        export const candidate_remove = async(e) => {
            let remove = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : true, 'body' : { 'remove' : true, 'id' : e.currentTarget.attributes[1].value }, 'data' : 'json' })
            if(remove){
                ServerData.pausePage({ 'msg' : 'Success!', 'timer' : false, 'run' : true, 'confirm' : true })
                retrieveContent(3,2,11)
            }
        ...
        export const delete_application = async(e) => {
            let delete_app = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : true, 'body' : { 'delete_jobs' : true, 'id' : e.currentTarget.attributes[1].value }, 'data' : 'json' })
        ...
        export const delete_jobs = async(e) => {
            ...
            let jobs = await ServerData.bindAuth({ 'method' : 'POST', 'link' : 'http://localhost/jobs/php/index.php', 'header' : true, 'body' : { 'destroy_jobs' : true, 'id' : id }, 'data' : 'json' })
        ...
        export const go_away = async(e) => {
            let out = await ServerData.bindAuth({ 'method' : 'POST', 'link' : `http://localhost/jobs/php/index.php`, 'header' : false, 'body' : { 'out' : true }, 'data' : 'json'})
            if(out.command){
                ServerData.pausePage({ 'msg' : 'Signed Out', 'run' : true, 'timer' : 'slow', 'confirm' : false })
                window.location.assign('http://localhost/jobs/index.html')
            }
        }
        export const delete_account = async(e) => {
            let destroy = await ServerData.bindAuth({ 'method' : 'POST', 'link' : `http://localhost/jobs/php/index.php`, 'header' : false, 'body' : { 'destroy' : true, 'table' : e }, 'data' : 'json'})
            if(destroy.command){
        ...
        export const createEmployer = async(e) => {
            ...
            if(!institution || !telephone || !email || !password || (password !== r_password)){
                ...
                let create = await ServerData.bindAuth({'method' : 'POST', 'link' : `http://localhost/jobs/php/index.php`, 'header' : true, 'body' : { 'create_employer' : true, 'institution' : institution, 'password' : password, 'email' : email, 'telephone' : telephone }, 'data' : 'json' })
                ...
                if(create){
                    ...
                    if(create.success)
                        setTimeout(
                            function(){
                                window.location.assign('http://localhost/jobs/#Account')
        ...
        export const createAccount = async(e) => {
            ...
            if(!name || !email || !password ){
                ...
                let create = await ServerData.bindAuth({'method' : 'POST', 'link' : `http://localhost/jobs/php/index.php`, 'header' : true, 'body' : { 'create' : true, 'name' : name, 'password' : password, 'email' : email }, 'data' : 'json' })
        ...
        export const ulog = async(e) => {
            ...
            if(!password || !telephone){
                ...
                let logIn = await ServerData.bindAuth({ 'method' : 'POST', 'link' : `http://localhost/jobs/php/index.php`, 'header' : true, 'body' : { 'login' : true, 'email' : telephone, 'password' : password }, 'data' : 'json' })
                if(logIn){
                    ...
                    if(logIn.identity){
                        setTimeout(
                            function(){
                                window.location.assign('http://localhost/jobs/account/index.html')
                            },2000
                        )
                    }
                }
        ...
        export const jobsContent = async(no_panel) => {
            let login = await ServerData.bindAuth({ 'method' : 'POST', 'link' : `http://localhost/jobs/php/index.php`, 'header' : true, 'body' : { 'user' : true }, 'data' : 'json' })
            if(login){
                let string = await ServerData.bindAuth({ 'method' : 'GET', 'link' : `http://localhost/jobs/php/string.php`, 'header' : true, 'data' : 'json', 'load' : true })
                if(string){
